...
class MetalExchangeWebClientIT implements WithAssertions {
    ...
    // The container image tag must track the mockserver-client-java version: MockServerClient
    // enforces a hard major.minor compatibility check against the running server.
    ...
    @AfterEach
    void tearDown() {
        // Reset expectations only; MockServerClient#close() would stop the server itself.
